  ...
  const fetchChartData = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(
          "soil_type, status, created_at, id, location, gravel_percent, sand_percent, fines_percent",
        )
      ...
      if (data && data.length > 0) {
        ...
        const disapprovedCount = data.filter(
          (d) => d.status?.toUpperCase() === "DISAPPROVED",
        ).length;
        const approvalRate =
          totalAnalyses > 0
            ? ((approvedCount / totalAnalyses) * 100).toFixed(1)
            : 0;
