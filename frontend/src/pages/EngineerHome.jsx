...
import { TestTube2, BarChart2, CheckCircle, AlertCircle, Clock, TrendingUp } from 'lucide-react';
import {
  BarChart,
  Bar,
  XAxis,
...
const EngineerHome = () => {
      ...
      try {
        ...
        if (user && user.id) {
          const { data: profile } = await supabase
            ...
            .eq('id', user.id)
            .single();
          setEngineerName(profile?.full_name || user.email || 'Engineer');
        } else {
  ...
  const fetchChartData = async () => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user || !user.id) throw new Error('Authentication error');
      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-12">
          <StatCard
            ...
            description="All submitted analyses"
            icon={TestTube2}
            color="blue"
          />
          <StatCard
