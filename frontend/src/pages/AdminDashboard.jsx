...
const AdminDashboard = () => {
    ...
    try {
      ...
      const usersWithRecordStatus = await Promise.all(
        (usersData || []).map(async (user) => {
          ...
          const hasRecords =
            !recordsError && recordsData && recordsData.length > 0;
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        showAlert(
          result?.detail || result?.message || "Failed to delete account.",
          "error",
        );
      } else {
          ...
          <table className="w-full table-auto border border-accent-400 dark:border-accent-600 rounded-xl shadow-lg">
            <thead>
              <tr className="bg-accent-100 dark:bg-accent-900/50">
                <th className="px-5 py-4 border-b text-left font-semibold text-gray-700 dark:text-gray-200">
                  Full Name
                </th>
                <th className="px-5 py-4 border-b text-left font-semibold text-gray-700 dark:text-gray-200">
                  Role
            ...
            <tbody className="divide-y divide-accent-200 dark:divide-accent-800">
              {currentUsers.map((user) => (
                <tr
                  key={user.id}
                  className="bg-white dark:bg-gray-800/50 hover:bg-accent-50 dark:hover:bg-accent-900/20 transition-colors"
                >
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100 dark:from-gray-900 dark:via-gray-900 dark:to-slate-900 text-gray-800 dark:text-gray-100 transition-colors duration-300">
      ...
      {alert.show && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 animate-in slide-in-from-top duration-300">
          <div
            className={`${alertStyles.bg} ${alertStyles.text} border-l-4 rounded-lg shadow-lg px-6 py-4 flex items-center gap-3 min-w-[320px] max-w-md backdrop-blur-sm`}
          >
            {alertStyles.icon}
      ...
      <main className="max-w-7xl mx-auto mt-12 px-4 pb-12">
        ...
        {/* User Table */}
        <div
          className="bg-white/95 dark:bg-gray-800/95 rounded-2xl shadow-xl border border-accent-700 dark:border-accent-600 p-8 transition-all duration-300"
          style={{ backdropFilter: "blur(4px)" }}
        >
