...
import { BarChart2, CheckCircle, AlertCircle, Clock, TrendingUp } from 'lucide-react';
import {
  BarChart,
  Bar,
  XAxis,
  ...
  return (
    <PageLayout currentPage="home" userType="expert">
      ...
      {!loading && !error && recentAnalyses.length > 0 && (
        <div className="bg-white/95 dark:bg-gray-800/95 rounded-2xl shadow-2xl p-10 mb-12 border border-accent-700 transition-all duration-500 animate-in fade-in" style={{ backdropFilter: 'blur(4px)' }}>
          <h3 className="text-3xl font-bold text-accent-900 dark:text-accent-200 mb-6">Recent Submissions</h3>
      ...
      <div className="bg-white/95 dark:bg-gray-800/95 rounded-2xl shadow-2xl p-10 border border-accent-700 transition-all duration-500 animate-in fade-in" style={{ backdropFilter: 'blur(4px)' }}>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <button onClick={fetchData} className="text-accent-700 dark:text-accent-300 hover:underline mt-4">
              Retry
            </button>
          </div>
        ) : (
              ...
              <div className="lg:col-span-2">
                ...
                {statusData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={420}>
                    <BarChart data={statusData} margin={{ top: 30, right: 40, left: 20, bottom: 80 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                      <XAxis dataKey="status" tick={{ fill: isDark ? '#d1d5db' : '#374151', fontSize: 14, fontWeight: 700 }}>
                        <Label value="Status" offset={20} position="bottom" fill={isDark ? '#d1d5db' : '#374151'} fontSize={14} fontWeight={700} />
                      </XAxis>
                      <YAxis tick={{ fill: isDark ? '#d1d5db' : '#374151', fontSize: 14, fontWeight: 700 }}>
                      ...
                      <Bar dataKey={showPercentage ? 'percentage' : 'count'} radius={[8, 8, 0, 0]} barSize={60}>
                  ...
                  <p className="text-center py-12 text-gray-700 dark:text-gray-200">No status data available.</p>
                )}
              </div>

              {/* Status Pie Chart */}
              <div>
                <h4 className="text-2xl font-bold mb-6 text-accent-900 dark:text-accent-200">Review Status</h4>
                {statusData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={420}>
                    <PieChart>
                      <Pie
                  ...
                  <p className="text-center py-12 text-gray-700 dark:text-gray-200">No review data available.</p>
                )}
              </div>
            </div>
              ...
              {soilTypeData.length > 0 ? (
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={soilTypeData} margin={{ top: 30, right: 40, left: 20, bottom: 100 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                    <XAxis dataKey="soil_type" tick={{ fill: isDark ? '#d1d5db' : '#374151', fontSize: 14, fontWeight: 700 }}>
                ...
                <p className="text-center py-12 text-gray-700 dark:text-gray-200">No soil type data available.</p>
              )}
            </div>
          </div>
        )}
