...
import { Sun, Moon, LogOut, Home, BarChart2, CheckCircle, AlertCircle, Clock, TrendingUp, PieChart as PieChartIcon } from 'lucide-react';
import {
  BarChart,
  Bar,
  XAxis,
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const totalApproved = data.filter(d => d.status?.toUpperCase() === 'APPROVED').length;
        const totalPending = data.filter(d => d.status?.toUpperCase() === 'PENDING' || !d.status).length;
        const totalDisapproved = data.filter(d => d.status?.toUpperCase() === 'DISAPPROVED').length;
        const approvalRate = totalAnalyses > 0 ? ((totalApproved / totalAnalyses) * 100).toFixed(1) : 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100 dark:from-gray-900 dark:via-gray-900 dark:to-slate-900 text-gray-800 dark:text-gray-100 transition-colors duration-300">
      ...
      <header className="bg-white/95 dark:bg-gray-800/95 shadow px-8 py-6 flex justify-between items-center border-b border-amber-700 transition-all duration-300" style={{ backdropFilter: 'blur(4px)' }}>
        <div
          ...
          title="Refresh Expert Home"
          tabIndex={0}
          role="button"
          onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') window.location.reload(); }}
          aria-label="Refresh Expert Home"
        >
      ...
      <main className="max-w-full mx-auto mt-12 px-6 pb-12">
        ...
        <div className="bg-white/95 dark:bg-gray-800/95 rounded-2xl shadow-2xl p-10 mb-12 border border-amber-700 transition-all duration-500 animate-in fade-in" style={{ backdropFilter: 'blur(4px)' }}>
          ...
          <p className="mb-8 text-lg text-gray-700 dark:text-gray-200">
            Review and validate soil analysis submitted by engineers to ensure accuracy and quality.
          </p>
          <button
        ...
        {!loading && !error && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-12">
            ...
            <div className="bg-white/95 dark:bg-gray-800/95 rounded-2xl shadow-xl p-6 border border-blue-400 dark:border-blue-600 transition-all duration-300 hover:shadow-2xl transform hover:scale-105">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Under review</p>
            </div>
            ...
            <div className="bg-white/95 dark:bg-gray-800/95 rounded-2xl shadow-xl p-6 border border-yellow-400 dark:border-yellow-600 transition-all duration-300 hover:shadow-2xl transform hover:scale-105">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-yellow-900 dark:text-yellow-200">Pending Review</h3>
                <Clock className="w-8 h-8 text-yellow-600 dark:text-yellow-400" />
              </div>
              <p className="text-4xl font-bold text-yellow-700 dark:text-yellow-300">{stats.totalPending}</p>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Awaiting decision</p>
            </div>

            {/* Approved */}
            <div className="bg-white/95 dark:bg-gray-800/95 rounded-2xl shadow-xl p-6 border border-green-400 dark:border-green-600 transition-all duration-300 hover:shadow-2xl transform hover:scale-105">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-green-900 dark:text-green-200">Approved</h3>
                <CheckCircle className="w-8 h-8 text-green-600 dark:text-green-400" />
              </div>
              <p className="text-4xl font-bold text-green-700 dark:text-green-300">{stats.totalApproved}</p>
            ...
            <div className="bg-white/95 dark:bg-gray-800/95 rounded-2xl shadow-xl p-6 border border-red-400 dark:border-red-600 transition-all duration-300 hover:shadow-2xl transform hover:scale-105">
              ...
              <p className="text-4xl font-bold text-red-700 dark:text-red-300">{stats.totalDisapproved}</p>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Requires revision</p>
            </div>
        ...
        {!loading && !error && recentAnalyses.length > 0 && (
          <div className="bg-white/95 dark:bg-gray-800/95 rounded-2xl shadow-2xl p-10 mb-12 border border-amber-700 transition-all duration-500 animate-in fade-in" style={{ backdropFilter: 'blur(4px)' }}>
            <h3 className="text-3xl font-bold text-amber-900 dark:text-amber-200 mb-6">Recent Submissions</h3>
        ...
        <div className="bg-white/95 dark:bg-gray-800/95 rounded-2xl shadow-2xl p-10 border border-amber-700 transition-all duration-500 animate-in fade-in" style={{ backdropFilter: 'blur(4px)' }}>
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
            <h3 className="text-3xl font-bold text-green-800 dark:text-green-200">Analysis Overview</h3>
            ...
            <div className="space-y-12">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="transition-all duration-500">
                  <h4 className="text-2xl font-bold mb-6 text-amber-900 dark:text-amber-200">Review Status</h4>
                  {statusData.length > 0 ? (
                    <ResponsiveContainer
                      width="100%"
                      height={420}
                      aria-label="Pie chart showing review status"
                    >
                      <PieChart>
                        <Pie
                          data={pieChartData}
                    ...
                    <div className="text-center py-12">
                      <p className="text-lg text-gray-700 dark:text-gray-200">No review data available.</p>
                    </div>
                  )}
                </div>
              </div>
